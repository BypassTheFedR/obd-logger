        ...
        if not connected:
            try:
                ...
                if connected:
                    ...
                    set_filtered_pids([pid for pid in PIDS_TO_WATCH if pid in supported])
        ...
        try:
            ...
            if response and response.value is not None:
                ...
                if hasattr(val, "magnitude"):
                    data[cmd.name] = val.magnitude
                else:
                    data[cmd.name] = str(val)
                logging.debug(f"[DATA] {cmd.name} = {data[cmd.name]}")
            else:
                data[cmd.name] = None
        except Exception as e:
            data[cmd.name] = None
